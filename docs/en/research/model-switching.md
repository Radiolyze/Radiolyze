...
**Prompt compatibility:** The system prompt in `backend/app/prompts.py` is tuned for MedGemma. If switching to a different model family, review and test the prompt — other models may require different formatting or instruction styles.
...
# Some installations use a wrapper; check backend/app/vllm_client.py
```
...
The backend automatically uses `guided_json` for vLLM. If you switch to a backend that does not support it, you may need to adjust `backend/app/vllm_client.py` to use `response_format` instead.
