...
Yes. The inference client (`backend/app/vllm_client.py`) uses an OpenAI-compatible API. Any model served by vLLM, Ollama, or a compatible endpoint can be substituted by changing the `INFERENCE_BASE_URL` and `INFERENCE_MODEL` environment variables.
