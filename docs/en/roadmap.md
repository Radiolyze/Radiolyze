...
5. Split the backend monoliths (#293) — `inference_clients.py` (774), `api/training.py` (752) and `api/inference.py` (615) are done, the last two now `app/services/training_export/` and `app/services/inference_queue/` behind routes-only modules; `tasks.py` (991) remains, behind #296.
