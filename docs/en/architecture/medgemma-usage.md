...
| Speech (MedASR) | in use | ASR service via vLLM; UI captures dictation | `backend/app/asr_providers.py`, `src/hooks/useASR.ts` |
...
- References: `docker-compose.yml`, `services/segmenter/app/medgemma_preprocess.py`,
  `backend/app/inference_clients/volume.py:generate_volume_inference_summary`.
