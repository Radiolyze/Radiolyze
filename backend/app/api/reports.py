...
def _serialize_comparison(comparison: ReportComparison) -> ReportComparisonResponse:
...
async def asr_transcript(
    ...
    try:
        result = await ASRService(db).transcribe(
            content,
    ...
    if result.persisted and report_id:
        await broadcast_status(
            report_id,
            {
...
def create_comparison(
    ...
    try:
        comparison = ComparisonService(db).create(report_id, payload)
    except NotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def list_comparisons(
    ...
    return [_serialize_comparison(c) for c in ComparisonService(db).list_for_report(report_id)]
...
def export_pdf(
    ...
    try:
        ...
        # UnsupportedExportError carries 501; a plain ValidationError, 400.
        raise HTTPException(status_code=exc.status_code, detail=str(exc)) from exc
