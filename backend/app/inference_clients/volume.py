"""Volume inference -- interpret a whole series rather than a single frame.

The segmenter service samples and windows the slices first; this module only
builds the prompt around that manifest and drives vLLM with the resulting
images. The ``preprocess_for_medgemma`` import stays inside the function so a
mock-only deployment never pulls the segmenter client in.
"""
...
from ..inference_utils import _compact_metadata, _env_flag, _env_float, _format_float
...
from ._common import _evidence_metadata, _raise_if_schema_invalid
...
def generate_volume_inference_summary(
    *,
    ...
    try:
        ...
        json_metadata = _evidence_metadata(
            parse_metadata,
            system_meta=system_meta,
            task_meta="volume_summary",
            evidence_indices=evidence_indices,
            confidence_label=confidence_label,
            has_images=True,
        )
        json_metadata["volume_preprocess"] = {
