"""Volume inference: sample a series into slices, then interpret them.

This family differs from :mod:`.text` in that the images do not arrive with
the request. The segmenter is asked to window and sample the series first,
and that preprocess step is a failure mode of its own — separate from the
vLLM call and with its own fallback.
"""
