...
This is the one family that does not go through ``vllm_client``'s request
helper: the streaming response has to be consumed line by line, so the
request is assembled here from the same base URL, headers and timeout the
