"""Longitudinal comparison -- current study against a prior one (P1.A).

Two volume preprocesses feed a single prompt. The slice manifest is what lets
the model cite evidence across both studies: current slices are indexed from 1
and prior slices continue the numbering, so one index space covers both.
"""
...
from ._common import _raise_if_schema_invalid
...
def generate_comparison_text(
    *,
    ...
    try:
        ...
        _raise_if_schema_invalid(metadata)
