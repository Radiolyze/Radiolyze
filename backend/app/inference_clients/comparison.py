"""Longitudinal comparison of a current study against a prior one (P1.A).

Two volume preprocesses feed one prompt: the current slices first, the prior
slices after them, with a manifest that offsets the prior indices so the model
can refer to either set unambiguously.
"""
