...
def test_create_qa_rule(client):
    response = client.post("/api/v1/qa-rules", json={
...
def test_update_qa_rule(client):
    create_resp = client.post("/api/v1/qa-rules", json={
        "name": "Min length",
    ...
    response = client.patch(f"/api/v1/qa-rules/{rule_id}", json={
...
def test_qa_check_with_custom_rules(client):
    ...
    client.post("/api/v1/qa-rules", json={
