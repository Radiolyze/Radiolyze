...
def test_generate_volume_summary_uses_preprocess(monkeypatch) -> None:
    ...
    with (
        ...
        patch("app.inference_clients.volume._vllm_chat_completion", side_effect=fake_chat),
    ):
...
def test_generate_comparison_text_emits_structured_output(monkeypatch) -> None:
    ...
    with (
        ...
        patch("app.inference_clients.comparison._vllm_chat_completion", return_value=fake_response),
    ):
