...
def test_request_peer_review(client, sample_report):
    ...
    resp = client.post(f"/api/v1/reports/{report_id}/request-review", json={
...
def test_list_peer_reviews(client, sample_report):
    ...
    report_id = sample_report["id"]
    client.post(f"/api/v1/reports/{report_id}/request-review", json={
        "assignedTo": "dr-schmidt",
    })
...
def test_submit_peer_review(client, sample_report):
    ...
    report_id = sample_report["id"]
    review_resp = client.post(f"/api/v1/reports/{report_id}/request-review", json={
        "assignedTo": "dr-schmidt",
    })
    review_id = review_resp.json()["id"]

    resp = client.post(f"/api/v1/reports/{report_id}/reviews/{review_id}/submit", json={
...
def test_submit_already_completed_review(client, sample_report):
    ...
    client.post(f"/api/v1/reports/{report_id}/reviews/{review_id}/submit", json={
        "reviewComment": "OK",
        "decision": "agree",
    })
    resp = client.post(f"/api/v1/reports/{report_id}/reviews/{review_id}/submit", json={
...
def test_request_review_nonexistent_report(client):
    ...
    resp = client.post("/api/v1/reports/nonexistent/request-review", json={
        "assignedTo": "dr-schmidt",
    })
