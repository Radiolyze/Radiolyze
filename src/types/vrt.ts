...
export interface VRTPreset {
  id: string;
  /** Product name of the preset — a radiology term, identical in every UI language. */
  name: string;
...
/**
 * Rendering parameters per preset. The human-readable description is *not*
 * here: it is looked up per render as `viewer:vrt.presets.<id>`, so a language
 * switch reformats a toolbar that is already on screen. `VRT_PRESETS` ids and
 * the keys under that group are kept in step by a contract test.
 */
