...
export function ImpressionPanel({
  ...
  const schemaName =
    typeof inferenceMeta?.schema_name === "string" ? inferenceMeta.schema_name : undefined;
  const schemaVersion =
    typeof inferenceMeta?.schema_version === "string" ? inferenceMeta.schema_version : undefined;
  const schemaValid =
    typeof inferenceMeta?.json_schema_valid === "boolean"
      ? inferenceMeta.json_schema_valid
      : undefined;
  const schemaError =
    typeof inferenceMeta?.json_error === "string" ? inferenceMeta.json_error : undefined;
      ...
      {hasInferenceResult && (
        <div className="px-4 py-2 border-b border-border bg-panel-secondary/20 space-y-2">
            ...
            {resolvedInferenceStatus && inferenceStatusLabel[resolvedInferenceStatus] && (
              <Badge
                ...
                className={cn(
                  ...
                  inferenceStatusLabel[resolvedInferenceStatus].className,
                )}
              >
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            ...
            {inferenceConfidence !== undefined && (
              <div>
                {t("inference.confidence", { value: Math.round(inferenceConfidence * 100) })}
              </div>
            )}
            {inferenceCompletedAt && (
              <div>
                {t("inference.completedAt", {
                  time: `${formatDate(inferenceCompletedAt)} ${formatTime(inferenceCompletedAt)}`,
                })}
              </div>
            )}
            {inferenceJobId && <div>Job: {inferenceJobId.slice(0, 8)}...</div>}
            {(schemaName || schemaVersion || schemaValid !== undefined) && (
              <div className="flex flex-wrap items-center gap-2">
                <span>
                  {t("inference.schema")}: {schemaName ?? "n/a"}
                ...
                {evidenceRefs.map((ref) => (
                  ...
                  >
                    <span className="truncate">
                      {ref.instanceId.slice(0, 8)}… ·{" "}
                      {t("inference.frame", { frame: ref.frameIndex })}
                    </span>
                  </Button>
                ))}
              </div>
