    ...
    >
        ...
        <div className="flex items-center gap-2">
          ...
          {onAnalyzeImages && (
            <Button
              ...
              className={cn("transition-all", isAnalyzing && "animate-pulse")}
            >
      ...
      {confidence > 0 && status === "idle" && (
        <div className="px-4 py-2 bg-panel-secondary/50 border-b border-border animate-fade-in">
          <ConfidenceBar value={confidence} label={t("ai.confidence")} size="sm" />
        </div>
      )}
        ...
        {isEditing ? (
          ...
          <div className="h-full p-4 overflow-y-auto animate-fade-in">
            {findings ? (
              ...
              <p className="text-sm text-muted-foreground italic">{t("findings.placeholder")}</p>
            )}
          </div>
        )}
      </div>
