...
import { splitCounts, verifiedPercentage } from "@/lib/trainingExport";
...
export function TrainingStatsGrid({ stats, isLoading, splitRatio }: TrainingStatsGridProps) {
  const { t } = useTranslation("training");

  const verified = verifiedPercentage(stats);
  const { train, validation } = splitCounts(stats, splitRatio);
    ...
    <div className="grid gap-6 md:grid-cols-3 mb-8">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-2 mt-2">
                <Progress value={verified} className="h-2 flex-1" />
                <span className="text-xs text-muted-foreground">
                  {t("stats.verifiedPercentage", { percent: verified })}
                </span>
              </div>
            </>
          )}
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex gap-4">
            <div>
              <div className="text-3xl font-bold text-primary">{train}</div>
              <div className="text-xs text-muted-foreground">{t("stats.training")}</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-muted-foreground">{validation}</div>
