...
export function ExportFormatCard({ selected, onSelect }: ExportFormatCardProps) {
        ...
        {FORMAT_ENTRIES.map(([format, info]) => (
          ...
          >
            <div
              className={cn(
                "p-2 rounded-lg",
                selected === format ? "bg-primary text-primary-foreground" : "bg-muted",
              )}
            >
            ...
            <div className="flex-1">
              <div className="font-medium flex items-center gap-2">
                {info.name}
                {selected === format && <CheckCircle2 className="h-4 w-4 text-primary" />}
              </div>
