...
function rgbToHex(rgb: RGB): string {
  return (
    "#" + rgb.map((channel) => toByte(channel).toString(16).padStart(2, "0").toUpperCase()).join("")
  );
}
  ...
  return (
    <div className="space-y-3 text-xs">
      <div className="flex items-center gap-2">
        <span aria-hidden className="h-6 w-6 rounded border" style={{ backgroundColor: hex }} />
      ...
      <ChannelSlider label={t("mesh.colorPicker.r")} value={toByte(r)} onChange={setChannel(0)} />
      <ChannelSlider label={t("mesh.colorPicker.g")} value={toByte(g)} onChange={setChannel(1)} />
      <ChannelSlider label={t("mesh.colorPicker.b")} value={toByte(b)} onChange={setChannel(2)} />
    </div>
  );
}
...
function ChannelSlider({ label, value, onChange }: ChannelSliderProps) {
  return (
    <div className="flex items-center gap-2">
      <span className="w-3 text-[10px] uppercase text-muted-foreground">{label}</span>
      <Slider
        value={[value]}
        min={0}
        max={255}
