  ...
  return (
    <div className={`relative h-full w-full ${className ?? ""}`}>
      ...
      <div className="absolute top-12 left-4 z-20 flex items-center gap-2 rounded-md border bg-card/90 p-2 backdrop-blur">
        ...
        <Button size="sm" onClick={handleStart} disabled={isStarting || isRunning || !studyUid}>
          {isStarting || isRunning ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : null}
          {t("mesh.generate")}
        </Button>
        ...
        <div className="ml-1 flex items-center gap-1 border-l pl-2">
          ...
          {clipEnabled && (
            <Select value={clipAxis} onValueChange={(v) => onClipAxisChange(v as "x" | "y" | "z")}>
      ...
      {isFinished &&
        ...
        (() => {
          ...
          const url =
            pushState.phase === "pushed" ? pushState.url : (status?.dicom_seg_orthanc_url ?? null);
          return (
      ...
      {status?.status === "failed" && (
        ...
        >
          <div className="flex-1">
            ...
            <div className="mt-0.5 break-words">{status.error || error?.message || "—"}</div>
          </div>
          <Button
            size="sm"
            variant="outline"
        ...
        <div className="absolute top-12 right-4 z-20 flex max-h-[85%] w-80 flex-col gap-2 rounded-md border bg-card/90 p-3 backdrop-blur">
          ...
          <ul className="space-y-3 overflow-y-auto pr-1">
            {displayedLabels.map((label) => {
              const state = labelStates[label.id] ?? defaultLabelState(label, { visible: false });
