...
vi.mock("@/services/wsClient", () => ({
  createWsClient: vi.fn((opts: { onMessage?: (data: unknown) => void; onOpen?: () => void }) => {
...
describe("useReportStatusSync", () => {
  ...
  it('shows a success toast when qaStatus is "pass"', () => {
    ...
    expect(toastMock).toHaveBeenCalledOnce();
    expect(toastMock).toHaveBeenCalledWith(expect.not.objectContaining({ variant: "destructive" }));
  });
  ...
  it("getEnhancedItems is a no-op for reports without live status", () => {
    ...
    const items = [{ report: { id: "rep-unknown", aiStatus: "queued" } }] as Parameters<
